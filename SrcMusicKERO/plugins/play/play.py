...
from SrcMusicKERO.utils import seconds_to_min, time_to_seconds
from SrcMusicKERO.utils.channelplay import get_channeplayCB
from SrcMusicKERO.utils.decorators.language import languageCB
from SrcMusicKERO.utils.decorators.play import PlayWrapper
from SrcMusicKERO.utils.formatters import formats
from SrcMusicKERO.utils.inline import (
    botplaylist_markup,
    livestream_markup,
    playlist_markup,
    slider_markup,
...
from SrcMusicKERO.utils.logger import play_logs
from SrcMusicKERO.utils.stream.stream import stream
...
async def download(bot_username, link, video: Union[bool, str] = None):
    ...
    def audio_dl():
        ydl_opts = {
            ...
            "outtmpl": f"Music/{bot_username}/%(id)s.%(ext)s",
            "geo_bypass": True,
    ...
    if video:
        ...
        if stdout:
            downloaded_file = stdout.decode().strip()  # إزالة المسافات الزائدة
        else:
            return None  # معالجة الأخطاء حسب الحاجة
    else:
...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            user_mention = message.from_user.mention if message.from_user.mention else None
            spotify = True
